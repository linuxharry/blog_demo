...
def paginator_data(paginator, page):
    ...
    if paginator.num_pages == 1:
        # 如果无法分页，也就是只有1页不到的内容，则无需显示分页导航条，不用任何分页导航条的数据，因此返回一个空字典
        return {}
    # 当前页左边连续的页码号，初始值为空
    left = []
    # 当前页右边连续的页码号，初始值为空
...
def index(request):
    ...
    page_data = paginator_data(paginator, page)
...
def archives(request, year, month):
    entries = models.Entry.objects.filter(create_time__year=year, create_time__month=month)
    page = request.GET.get('page', 1)
    entry_list, paginator = make_paginator(entries, page)
    page_data = paginator_data(paginator, page)
    return render(request, 'blog/index.html', locals())
...
def login(request):
    ...
    if code is None:
        return redirect('/')
    access_token_url = 'https://api.weibo.com/oauth2/authorize?client_id=%s&client_secret=%s' \
                       'grant_type=authorization_code&response_type=code&redirect_uri=http://www.adaiblog.com/' \
                       'login&code=%s' % (settings.CLIENT_ID, settings.APP_SECRET, code)
    ...
    user_info_url = 'https://api.weibo.com/2/users/show.json?accesss_token=%s&uid=%s' % (token, uid)
    user_info = requests.get(user_info_url)
    user_info_dict = json.loads(user_info.text)
    request.session['screen_name'] = user_info_dict['secreen_name']
